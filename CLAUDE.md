...
The core is a pure sentence-building pipeline in `src/lib/`; `src/App.svelte` only constructs a `SentenceParams` object, calls `buildSentence`, and renders the resulting `Word[]`.
...
`yarn run check` currently reports 5 `Object is possibly 'undefined'` errors (`Verb.ts:41`, `BeVerb.ts:17,20,21,29`), all from `this.subject` being optional in `Verb`. They don't affect `yarn dev`/`yarn build`.
